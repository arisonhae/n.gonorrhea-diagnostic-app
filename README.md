...
전체 이미지 데이터(약 2.8 GB)는 Zenodo 에 있다:
[10.5281/zenodo.22090185](https://doi.org/10.5281/zenodo.22090185)

위 DOI 는 항상 최신 버전(현재 v2, 2026-08-28)으로 연결된다. **분석을 재현하려면 `dataset.zip`
하나만 받으면 된다.** 나머지는 촬영 원본과 중간 산출물이며, 특정 실험을
다시 들여다볼 때만 필요하다. 파일별 설명은 Zenodo 레코드의 README 에 있다.
...
pilot 산출물은 프리뷰 이미지가 많아 229 MB 에 달하므로 저장소가 아니라
...
적용하지 않은 별도 버전(v10)을 생성해 300장임을 확인했다. 이 버전은 Zenodo 의
`yolo_v10_original_300.zip` 으로 함께 공개되어 있다.
...
| 대상 | 라이선스 |
|---|---|
| 코드 (이 저장소) | **MIT** — [LICENSE](LICENSE) |
| 이미지 데이터셋 (Zenodo) | **CC BY 4.0** |

둘 다 출처를 밝히면 자유롭게 쓸 수 있다.
